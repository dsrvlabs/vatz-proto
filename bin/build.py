...
def _make_build_environment(output_dir, code):
    if code == 'python':
        ...
        # Copy dsrv.__init__.py
        src = os.path.join(TEMPLATE_DIR, 'python', 'dsrv.tmpl')
        dst = os.path.join(output_dir, code, 'dsrv', '__init__.py')

        if not os.path.exists(dst):
            shutil.copyfile(src, dst)

        # Copy dsrv.api.__init__.py
        src = os.path.join(TEMPLATE_DIR, 'python', 'dsrv.api.tmpl')
        dst = os.path.join(output_dir, code, 'dsrv', 'api', '__init__.py')

        if not os.path.exists(dst):
            shutil.copyfile(src, dst)

        api_root_dir = os.path.join(output_dir, code, 'dsrv', 'api')
...
def _python_compile(proto_file, output_path, proto_path_list, debug):
    #cmd = ['protoc', '-I=.', '--go_out', '.',  '--go_opt', 'paths=source_relative', '--go-grpc_out','.','--go-grpc_opt', 'paths=source_relative']
...
def build(params):
    ...
    for code in _get_generate_codes(params['code']):
        code_key = 'artifact_dir' if code == 'json' else 'output_dir'
        _make_output_path(params[code_key], code)

        for proto_file in proto_files:
            _compile_code(params, code, proto_file)

        _make_build_environment(params[code_key], code)
